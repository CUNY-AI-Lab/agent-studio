...
Guidelines for creating distinctive, production-grade frontend interfaces that avoid generic
"AI slop" aesthetics. Use this skill whenever you build an HTML artifact for the user.

## Workflow in this runtime

Write a **self-contained .html workspace file** (all CSS and JS inline), then surface it on
the canvas with the `ui_show_file` tool. The file renders in the user's browser, so external
resources like Google Fonts load normally — but keep logic and styling inline so the file
stands alone as a durable artifact.

For large pages, build the file incrementally inside codemode with `state.writeFile` and
`state.appendFile` rather than embedding the whole document in one tool argument. Then call
`ui_show_file` so the user sees the rendered page instead of source text.
...
<head>
  ...
  <title>Campus research dashboard</title>
  <style>
    /* Your CSS here */
  </style>
</head>
...
Write it to a workspace file (e.g., `report.html`), then call `ui_show_file` with that path.
...
No two interfaces should look the same. Vary between light and dark themes, different fonts,
different aesthetics. Each creation should be memorable and unique.

When the page presents research results, keep sources visible in the design: DOIs, arXiv
IDs, and catalog links belong in the layout as first-class elements, not buried footnotes.
